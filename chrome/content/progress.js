...
var CheckPlacesProgress = {
		...
		function recurseTree(container, checkContents) {
			...
			for (var i=0; i<container.childCount; i++) {
				...
				if (PlacesUtils.nodeIsQuery(child)) {
					...
					if (!excluded) cpp.livemarkCount++;
					isEmpty = false;
					var feedURI = null;
					var siteURI = null;
					if (child.itemId) {
						try {
							feedURI = PlacesUtils.livemarks.getFeedURI(child.itemId);
						} catch(e) {
							if (!excluded) {
								if (!cpp.cancelled) Components.utils.reportError(e);
								cpp.addItemToList("bad", cpp.bundle.GetStringFromName('bad_feeduri'), "", child.itemId);
							}
						}
						try {
							siteURI = PlacesUtils.livemarks.getSiteURI(child.itemId);
						} catch(e) {
							if (!excluded) {
								if (!cpp.cancelled) Components.utils.reportError(e);
								cpp.addItemToList("bad", cpp.bundle.GetStringFromName('bad_siteuri'), "", child.itemId);
							}
						}
					}
					...
					if (feedURI) {
						if (excluded) {
							cpp.addItemToList("excluded", excludedString, feedURI.spec, child.itemId);
							continue;
						}
						else if (notDuplicate(feedURI.spec, child) && checkLinks) {
							cpp.checkLink(cpp.useHead, child, feedURI.spec);
						}
					}
					else if (excluded) {
	...
	//Check the Places database (Synchronously, so may hang firefox)
	...
	onAlertLoad: function() {
		...
		if (exception) {
			...
			try {
				...
				version = em.getItemForID("checkplaces@andyhalford.com").version;
			} catch(e) {
			}
			message = message + version + "%0A%0A" + exception.toString() + "%0A%0APrefs:";
	...
	callBack: function(cancelled, status, channel, uri, child, head) {
		var bundle = Components.classes["@mozilla.org/intl/stringbundle;1"]
										.getService(Components.interfaces.nsIStringBundleService)
										.createBundle("chrome://checkplaces/locale/checkplaces.properties");
		var cpp = CheckPlacesProgress;
		if (cpp.debug) Components.utils.reportError("Checked link: " + uri);
		...
		if (cancelled) {
			...
			try {
				...
				//Other codes are passes, so do favicon check
				else if (cpp.favicons) {
					var faviconURL = null;
					try {
